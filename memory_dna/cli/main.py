...
from memory_dna.red_teamer import RedTeamer
...
app = typer.Typer(name="memory_dna")
...
def _show_version(value: bool):
    ...
    if value:
        typer.echo(f"memory_dna version {__version__}")
        raise typer.Exit()
...
def main(
    ...
    """
    memory_dna CLI for red teaming LLMs.
    """
    pass
...
def scan(
    ...
    provider: Optional[str] = typer.Option(
        ...
        help=(
            ...
            "CURSOR_API_KEY=cursor), else memory_dna's built-in scanner (OpenAI API)."
        ),
    ),
    ...
    comment: bool = typer.Option(
        False,
        "--comment",
        help="Post findings to Confident AI so memory_dna[bot] comments on the PR (GitHub Actions only).",
    ),
