...
from memory_dna.vulnerabilities.constants import VULNERABILITY_CLASSES_MAP
from memory_dna.metrics.types import EvaluationExample
from memory_dna.frameworks.api import (
    APIAttackMethod,
...
from memory_dna.attacks.single_turn.constants import (
    SINGLE_TURN_ATTACK_CLASSES_MAP,
)
from memory_dna.attacks.multi_turn.constants import MULTI_TURN_ATTACK_CLASSES_MAP
...
def _build_vulnerability(
    ...
    if vulnerability_class is None:
        # A name memory_dna doesn't know is either a custom vulnerability defined
        # on Confident AI (which carries the criteria needed to judge it) or a
        # vulnerability newer than the installed memory_dna.
        if not api_vulnerability.criteria:
            raise ValueError(
                f"Vulnerability '{api_vulnerability.name}' in the '{framework_name}' framework "
                "is not supported by this version of memory_dna. Upgrade with "
                "'pip install -U memory_dna', or define it as a custom vulnerability on Confident AI."
            )
    ...
    if unsupported_types:
        raise ValueError(
            ...
            "this version of memory_dna. Upgrade with 'pip install -U memory_dna'."
        )
...
def _build_attack(api_attack_method: APIAttackMethod) -> Optional[BaseAttack]:
    ...
    if attack_class is None:
        logging.warning(
            ...
            "version of memory_dna. Upgrade with 'pip install -U memory_dna' to run it."
        )
        return None
...
def build_risk_categories(
    ...
    """
    Rebuild a Confident AI framework's risk categories as memory_dna objects.

    Vulnerabilities and attack methods are resolved by name, which is how
    Confident AI stores them (its catalog is generated from memory_dna's own
